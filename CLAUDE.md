...
- **Fotoverkleinerung (v0.12.1):** `addPhotos()` skaliert jedes Foto über ein `<canvas>` auf max. 1600px
  (JPEG 0.8), bevor es in den State wandert – ein 3-4MB-Handyfoto landete vorher unverändert als Data-URL
  im selben JSON-Blob. Verarbeitung ist **sequenziell**, damit die Reihenfolge der ausgewählten Dateien
...
| `resizeImage(file)` | Foto aus `<input type=file>` auf 1600px/JPEG 0.8; bei Fehler bleibt das Original |
| `resizeDataURL(src)` | Dasselbe für bereits gespeicherte Fotos (Data-URL statt File) |
| `shrinkAllPhotos()` | Rechnet alle vorhandenen Fotos herunter, sichert vorher automatisch |
...
1. **localStorage ~5MB** – seit v0.12.1 deutlich entschärft: Fotos werden beim Hinzufügen auf 1600px/JPEG 0.8 verkleinert. Sichtbarkeit bleibt über Speicherplatz-Anzeige + Fehler-Banner.
...
- [x] Fotoverkleinerung beim Hinzufügen + Bestandsrettung „Bestehende Fotos verkleinern“ (v0.12.1)
